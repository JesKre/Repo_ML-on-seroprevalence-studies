...
folder <- "S:/PROJACTIVE/MUSPAD-LABORERGEBNISSE/LOKI_ML/muspad-ml-loki-sp2.1/Compare_models/Graphics/Predicted_values/14day_Pred"
...
metrics_df <- bind_rows(results, .id = "Model") %>%
  mutate(Model = gsub("_14day", "", Model))
...
write.csv(mape_df_new, "MAPE_Table_full_Data_14dayPred.csv")
write.csv(smape_df_new, "SMAPE_Table_full_Data_14dayPred.csv")
write.csv(rmsle_df_new, "RMSLE_Table_full_Data_14dayPred.csv")
write.csv(rmse_df_new, "RMSE_Table_full_Data_14dayPred.csv")
