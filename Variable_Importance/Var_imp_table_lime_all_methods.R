...
(file <- tail(list.files(pattern = "Var_Imp_lime_low_inc"), n=1))
lime_low <- read.csv2(file)[,-1]

MLP_lime_low <- lime_low %>%
  rename(MLP_low_Feature = feature,
         MLP_low_LIME_Value_mean = effect_mean,
         MLP_low_mean_overall = mean_overall,
         MLP_low_mean_cluster = mean_cluster) %>%
  select(-beta_mean, -x.recoded_mean)


(file <- tail(list.files(pattern = "Var_Imp_lime_high_inc"), n=1))
lime_high <- read.csv2(file)[,-1]

MLP_lime_high <- lime_high %>%
  rename(MLP_high_Feature = feature,
         MLP_high_LIME_Value_mean = effect_mean,
...
LSTM_7day_lime_low <- read.csv2("lime_tplus7_allMean_orig_scale_low.csv", fileEncoding = "latin1")[,-1]

LSTM_7day_lime_low <- LSTM_7day_lime_low %>%
  rename(LSTM_7day_low_Feature = Feature,
         LSTM_7day_low_LIME_Value_mean = LIME_Value_mean,
         LSTM_7day_low_mean_overall = mean_overall,
         LSTM_7day_low_mean_cluster = mean_cluster)


LSTM_7day_lime_high <- read.csv2("lime_tplus7_allMean_orig_scale_high.csv", fileEncoding = "latin1")[,-1]

LSTM_7day_lime_high <- LSTM_7day_lime_high %>%
  rename(LSTM_7day_high_Feature = Feature,
         LSTM_7day_high_LIME_Value_mean = LIME_Value_mean,
         LSTM_7day_high_mean_overall = mean_overall,
         LSTM_7day_high_mean_cluster = mean_cluster)

which(duplicated(LSTM_7day_lime_high$LSTM_7day_high_Feature))
...
LSTM_14day_lime_low <- read.csv2("lime_tplus14_allMean_orig_scale_low.csv", fileEncoding = "latin1")[,-1]

LSTM_14day_lime_low <- LSTM_14day_lime_low %>%
  rename(LSTM_14day_low_Feature = Feature,
         LSTM_14day_low_LIME_Value_mean = LIME_Value_mean,
         LSTM_14day_low_mean_overall = mean_overall,
         LSTM_14day_low_mean_cluster = mean_cluster)


LSTM_14day_lime_high <- read.csv2("lime_tplus14_allMean_orig_scale_high.csv", fileEncoding = "latin1")[,-1]

LSTM_14day_lime_high <- LSTM_14day_lime_high %>%
  rename(LSTM_14day_high_Feature = Feature,
         LSTM_14day_high_LIME_Value_mean = LIME_Value_mean,
         LSTM_14day_high_mean_overall = mean_overall,
         LSTM_14day_high_mean_cluster = mean_cluster)

which(duplicated(LSTM_14day_lime_high$LSTM_14day_high_Feature))
...
Important_Var_Coef <- cbind(MLP_lime_low[1:50,], MLP_lime_high[1:50,], 
                            LASSO_Coef[1:50,], LSTM_7day_lime_low[1:50,], 
                            LSTM_7day_lime_high[1:50,], LSTM_14day_lime_low[1:50,], 
                            LSTM_14day_lime_high[1:50,], VAR_Coef_p7[1:50,],
...
save(Important_Var_Coef, file = paste0("Variable_importance_lime_comparison_",Sys.Date(),".RData"))
write.csv2(Important_Var_Coef, file = paste0("Variable_importance_lime_comparison_",Sys.Date(),".csv"), fileEncoding = "latin1")
...
setwd("S:\\PROJACTIVE\\MUSPAD-LABORERGEBNISSE\\LOKI_ML\\muspad-ml-loki-sp2.1\\Variable_importance")
load(file = tail(list.files(pattern = glob2rx("Variable_importance_lime_comparison_*.RData")), n=1))
# Important_Var_Coef
...
Important_Var_Coef_lime <- Important_Var_Coef %>%
  ...
  mutate(MLP_low_LIME_Value_mean = round(MLP_low_LIME_Value_mean, digits = 2),
         MLP_high_LIME_Value_mean = round(MLP_high_LIME_Value_mean, digits = 2),
         ...
         LSTM_7day_low_LIME_Value_mean = round(LSTM_7day_low_LIME_Value_mean, digits = 2),
         LSTM_7day_high_LIME_Value_mean = round(LSTM_7day_high_LIME_Value_mean, digits = 2),
         LSTM_14day_low_LIME_Value_mean = round(LSTM_14day_low_LIME_Value_mean, digits = 2),
         LSTM_14day_high_LIME_Value_mean = round(LSTM_14day_high_LIME_Value_mean, digits = 2)
  )

cols <- c(grep("mean_overall", colnames(Important_Var_Coef_lime)),
          grep("mean_cluster", colnames(Important_Var_Coef_lime)))

for(i in cols){
  Important_Var_Coef_lime[,i] <- round(Important_Var_Coef_lime[,i], digits = 2)
}


setwd("S:\\PROJACTIVE\\MUSPAD-LABORERGEBNISSE\\LOKI_ML\\muspad-ml-loki-sp2.1\\Variable_importance")
save(Important_Var_Coef_lime, file = paste0("Variable_importance_lime_comparison_with_value_and_title_",Sys.Date(),".RData"))
write.csv2(Important_Var_Coef_lime, file = paste0("Variable_importance_lime_comparison_with_value_and_title_",Sys.Date(),".csv"), fileEncoding = "latin1")
